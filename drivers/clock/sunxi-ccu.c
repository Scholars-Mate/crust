 ...
 *
 * Order the iterations to favor changing the P divider over the PD divider,
 * prefer changing the PD divider over the M divider (since use of the M
 * divider is sometimes recommended against, and the PD and M dividers do the
 * same thing), and prefer changing all dividers over changing the parent.
 *
             ...
             struct sunxi_ccu_factors *factors, uint32_t rate)
{
	...
	for (size_t i = 0; i < 2 * SUNXI_CCU_PARENT_MAX; ++i) {
		...
		for (uint8_t m = 0; m < m_max; ++m) {
			/* The division function is optimized for doing x/1. */
			uint32_t rate1 = parent_max / (m + 1);
			for (uint8_t pd = 0; pd < pd_max; ++pd) {
				uint32_t rate2 = rate1 / (pd + 1);
				for (uint8_t p = 0; p < p_max; ++p) {
					int32_t  error;
					uint32_t new_rate = rate2 >> p;
